...
def build_station_risk(
    ...
    for record in records:
        ...
        for item in fail_items:
            ...
            if record.get("model"):
                group["models"].add(record.get("model"))

            if record.get("test_mode"):
                group["modes"].add(record.get("test_mode"))

            if record.get("station"):
                group["stations"].add(record.get("station"))
    ...
    for key, group in groups.items():
        ...
        if fail_count >= min_fail_count and sn_count >= min_sn_count:
            ...
            risks.append(
                {
                    "time_window": f"{bucket} / {window_minutes}min",
                    "item": item_name,
                    "signal": signal,
                    "instrument": instrument,
                    "fail_count": fail_count,
                    "sn_count": sn_count,
                    "models": sorted(list(group["models"])),
                    "modes": sorted(list(group["modes"])),
                    "stations": sorted(list(group["stations"])),
                    "latest_time": group["latest_time"],
                    "level": level,
                    "message": "同一时间窗口内多个 SN 出现相同或相近测试项 FAIL，可能与测试系统、夹具、探针、仪表链路或测试时序相关。建议工程师复核机台状态。该提示仅为风险分析，不作为故障结论。",
    ...
    risks.sort(
        key=lambda item: (
            0 if item.get("level") == "HIGH" else 1,
